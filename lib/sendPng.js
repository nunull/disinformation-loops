...
function sendPngThrottled (imageData) {
	...
	const timeMinutes = (chunkCount * config.throttleTimeout) / 1000 / 60
	const timeMinutesRounded = Math.round(timeMinutes * 100) / 100
	log.info(`sending png in throttled chunks with ${config.throttleTimeout} ms timeouts (approx. ${timeMinutesRounded} min)`)
